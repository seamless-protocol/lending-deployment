...
// import {RewardsDistributor} from 'seamless/aave-v3-periphery/rewards/RewardsDistributor.sol';
import {IRewardsController} from 'seamless/aave-v3-periphery/rewards/interfaces/IRewardsController.sol';
import {ITransferStrategyBase} from 'seamless/aave-v3-periphery/rewards/interfaces/ITransferStrategyBase.sol';
import {RewardsDataTypes} from 'seamless/aave-v3-periphery/rewards/libraries/RewardsDataTypes.sol';
import {IEACAggregatorProxy} from 'seamless/aave-v3-periphery/misc/interfaces/IEACAggregatorProxy.sol';
 ...
 * @author Aave
 **/
contract RewardsControllerAdminFunctions is RewardsDistributor, VersionedInitializable, IRewardsController {
  ...
  function getUserAssetRewardIndexAndAccrued(
    address user,
    address[] calldata buggedAssets,
    address[] calldata buggedRewards
  ) external view returns (
    uint256[] memory userIndexes,
    uint256[] memory accrueds
  )
  {
    userIndexes = new uint256[](buggedAssets.length);
    accrueds = new uint256[](buggedAssets.length);
    for(uint256 i=0; i<buggedAssets.length; i++) {
      address asset = buggedAssets[i];
      address reward = buggedRewards[i];

      accrueds[i] = _assets[asset].rewards[reward].usersData[user].accrued;
      ...
      userIndexes[i] = userIndex;
    }

    return(userIndexes, accrueds);
  }

  // struct UserIndexAccrued {
  //   address asset;
  //   address reward;
  //   uint256[] userIndex;
  //   uint256[] accrued;
  ...
  // ) external view returns (
  //   UserIndexAccrued[] memory indexesAndAccrueds
  // )
  // {
  //   indexesAndAccrueds = new UserIndexAccrued[](assets.length);
  //   for(uint256 i=0; i<assets.length; i++) {
  //     address asset = assets[i];

  //     indexesAndAccrueds[i].asset = asset;

  //     uint256 numAvailableRewards = _assets[asset].availableRewardsCount;

  //     indexesAndAccrueds[i].reward = new address[](numAvailableRewards);
  //     indexesAndAccrueds[i].userIndex = new uint256[](numAvailableRewards);
  //     indexesAndAccrueds[i].accrued = new uint256[](numAvailableRewards);

  //     for(uint128 r=0; r<numAvailableRewards; r++) {
  //       address reward = _assets[asset].availableRewards[r];

  //       indexesAndAccrueds[i].reward[r] = reward;
  //       indexesAndAccrueds[i].accrued[r] = _assets[asset].rewards[reward].usersData[user].accrued;
  ...
  //         userIndex = _assets[asset].rewards[reward].usersData[user].index_deprecated * PRECISION / assetUnit;
  //       }
  //       indexesAndAccrueds[i].userIndex[r] = userIndex;
  //     }
  //   }

  //   return indexesAndAccrueds;
  // }
  ...
  function setUserData(address asset, address reward, address user, uint256 index, uint128 accrued) external onlyTemporaryOverrideAdmin {
    RewardsDataTypes.RewardData storage rewardData = _assets[asset].rewards[reward];

    rewardData.usersData[user].index = index;
    rewardData.usersData[user].accrued = accrued;

    emit Accrued(asset, reward, user, rewardData.index, index, accrued);
  }
  ...
  function adminHandleAction(address user, uint256 totalSupply, uint256 userBalance, address asset) external {
  ...
  function adminClaimRewards(
    ...
    require(to != address(0), 'INVALID_TO_ADDRESS');
    return _claimRewards(assets, amount, user, user, to, reward);
  }
  ...
  function adminClaimAllRewards(
    address[] calldata assets,
    address to,
    address user
  ) external returns (address[] memory rewardsList, uint256[] memory claimedAmounts) {
    require(to != address(0), 'INVALID_TO_ADDRESS');
    return _claimAllRewards(assets, user, user, to);
  }
  ...
  function _claimAllRewards(
    ...
    for (uint256 i = 0; i < assets.length; i++) {
      console.log(i, "of", assets.length - 1);
      address asset = assets[i];
    ...
    for (uint256 i = 0; i < rewardsListLength; i++) {
      console.log("~", i, "of", rewardsListLength - 1);
